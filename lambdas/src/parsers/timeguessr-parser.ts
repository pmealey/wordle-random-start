...
export class TimeGuessrParser extends ResultParser {
  readonly countWinner = true;
  readonly gameName = 'TimeGuessr';
  readonly golfScoring = false;
  readonly helpText = null;
  readonly url = 'https://timeguessr.com';
  
  // Matches "TimeGuessr #401 17,398/50,000"
  // Note: C# uses [\d|,|.] which incorrectly includes literal pipes - fixed here to [\d,.]
  protected readonly parser = new RegExp(
    `${this.gameName} #\\d+ (?<score>[\\d,.]+)`,
    's'
  );
